...
export function dateToString (date, format = 'www - dd mmmm yyyy hh:mm') {
  ...
  let weekDayNameLong = weekDaysLong[weekday];
  let weekDayNameShort = weekDaysShort[weekday];

  switch (format) {
    case 'www - dd mmmm yyyy hh:mm':
      return `${weekDayNameLong} - ${day} ${months[month - 1]} ${year}, ${hour}:${minute}`;

    case 'ww - dd mmmm yyyy':
      return `${weekDayNameShort} - ${day} ${months[month - 1]} ${year}`;
